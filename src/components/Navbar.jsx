  ...
  return(
    <div>
      <Menu as='div' className="inline-block text-left">
        <div className='flex'>
          <Menu.Button className="inline-flex gap-2 w-full rounded-md bg-white
          md:px-4 py-2 text-sm font-medium text-slate-700 hover:bg-opacity-20">
...
<p className='text-sm font-semibold'>{user?.firstName ?? user?.name}</p>
        ...
        >
            <Menu.Items className="absolute z-50 right-2 mt-2 w-56 origin-top-right divide-y dividfe-gray-100
        ...
        <nav className='container mx-auto flex items-center justify-between p-5'>
          ...
          <ul className='hidden lg:flex gap-10 text-base'>
              ...
              <div className='hidden lg:block'>
                  {!user?.token ?(<Link to="/user-auth">
                    <CustomButton title="Sign In" 
                    containerStyles='
                    text-blue-600
                     py-1.5 px-5 focus:outline-none 
                     hover:bg-blue-700 hover:text-white
                    rounded-full text-base border border-blue-600'/>
                    
                  </Link>):(<div><MenuList user={user}/></div>)}
              </div>
              <button
